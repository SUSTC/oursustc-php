...
Class upload {
		...
		if(!is_array($attach) || empty($attach) || !$this->is_upload_file($attach['tmp_name']) || trim($attach['name']) == '' || $attach['size'] == 0) {
			...
			switch ($type) {
				...
				case 'music':
					...
					break;
				case 'document':
					if (!$this->is_document_ext($attach['ext'])) {
						$this->errorcode = -102;
						return false;
					}
					break;
	...
	function save($ignore = 0, $uid = 0) {
		...
		if ($this->type == 'avatar') {
			...
			$att = array(
				'uid' => $_G['uid'],
				'filename' => $this->attach['name'],
				'size' => $this->attach['size'],
				'uploadtime' => TIMESTAMP,
				'ext' => $this->attach['ext'],
				'type' => $this->type,
				'savepath' => $this->attach['attachdir']
			);

			//$this->attid = C::t('attachment')->insert($att, true)
			if (!($this->attid = DB::insert('attachment', $att, true))) {
				$this->errorcode = -103;
				return false;
			}
		...
		$this->attach['target'] = SC_ROOT.'./data/attach/'.$this->attach['attachment'];
	...
	function is_document_ext($ext) {
		static $docext  = array('doc', 'docx', 'pdf', 'xps', 'oxps');
		return in_array($ext, $docext) ? 1 : 0;
	}
	...
	function check_dir_type($type) {
		return !in_array($type, array('image', 'music', 'document', 'temp', 'avatar')) ? 'temp' : $type;
	}
