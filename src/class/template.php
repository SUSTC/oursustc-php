...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		$basefile = basename(SC_ROOT.$tplfile, '.htm');
		$file == 'common/header' && defined('CURMODULE') && CURMODULE && $file = 'common/header_'.CURMODULE;
		$this->file = $file;

		if($fp = @fopen(SC_ROOT.$tplfile, 'r')) {
			$template = @fread($fp, filesize(SC_ROOT.$tplfile));
			fclose($fp);
		} elseif($fp = @fopen($filename = substr(SC_ROOT.$tplfile, 0, -4).'.php', 'r')) {
		...
		$template = "<? if(!defined('IN_SUSTC')) exit('Access Denied'); {$headeradd}?>\n$template";
		...
		if(!@$fp = fopen(SC_ROOT.$cachefile, 'w')) {
			$this->error('directory_notfound', dirname(SC_ROOT.$cachefile));
		}
	...
	function languagevar($var) {
		return "<?php echo lang('template', '$var'); ?>";
	}

	function languagevar_old($var) {
		$vars = explode(':', $var);
		...
		if(!isset($langvar[$var])) {
			...
			if(!$isplugin) {
				...
				if (file_exists(SC_ROOT.'./src/lang/'.$path.'/template')) {
				...
				if(empty($_G['config']['plugindeveloper'])) {
					...
					if(@include(SC_ROOT.'./data/plugindata/'.$vars[0].'.lang.php')) {
	...
	function loadcsstemplate() {
		...
		$content = @implode('', file(SC_ROOT.'./data/cache/style_'.STYLEID.'_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen(SC_ROOT.'./data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
